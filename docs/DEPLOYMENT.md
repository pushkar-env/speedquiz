...
> **Redeploy the Worker for multiplayer.** Live matches open a WebSocket, and
> the original Worker rewrapped every response in `new Response(body, init)` —
> which silently discards the `webSocket` handle, so the client sees a 101 with
> no socket behind it. The fix is in `worker.js`; without redeploying, live
> battles fail through the public URL while working perfectly against Railway
> directly, which is a confusing thing to debug from a bug report.
...
- [ ] Worker **redeployed** since the WebSocket passthrough fix (live matches)
- [ ] Multiplayer env set (see `.env.example`); `FCM_SERVICE_ACCOUNT_JSON` if push is wanted
- [ ] `wss://<worker>/api/v1/multiplayer/realtime/ws` upgrades rather than 502ing
